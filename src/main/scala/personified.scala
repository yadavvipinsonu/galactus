...
val tunes_data = spark.read.parquet("hdfs:///user/viyadav/galactus/tunes_data_adj")
...
val t3 =  spark.sql(s"""SELECT  node_name,time_id,sum(local_15_minute_num_seconds) as rawmins, count(DISTINCT as_hh_no) as rawhhs,
...
AND tvh_df.node_name in ('SportsCenter 2017-10-17 on ESPN','Jimmy Kimmel Live! 10/16 11:35P M (62) on ABC','Jimmy Kimmel Live! 10/17 11:35P T (62) on ABC','Jimmy Kimmel Live! 10/18 11:35P W (62) on ABC','Jimmy Kimmel Live! 10/19 11:35P R (62) on ABC','Jimmy Kimmel Live! 10/20 11:35P F (62) on ABC','Jimmy Kimmel Live! 2017-10-16 on ABC','Jimmy Kimmel Live! 2017-10-17 on ABC','Jimmy Kimmel Live! 2017-10-18 on ABC','Jimmy Kimmel Live! 2017-10-19 on ABC','Jimmy Kimmel Live! 2017-10-20 on ABC','Jimmy Kimmel Live! on ABC'  )
group by node_name,time_id  order by time_id,node_name """)
t3.createOrReplaceTempView("t3")

val t33 =  spark.sql("""select node_name,depth, sum(proj_mins) as proj_mins ,sum(rawhhs),CAST(sum(proj_hhs) as DECIMAL(38,4) ),
...
val t44det =  spark.sql("select t33.node_name,cast(proj_mins/60 as long) as proj_mins ,waf,rawhhs,CAST(rawhhs*waf  as DECIMAL(18,4)) as proj_hhs  from t33, t4 WHERE t33.node_name = t4.node_name" ).cache(); 
...
FROM tunes_data tv_df, tvh_df WHERE tv_df.web_id = tvh_df.web_id AND tvh_df.node_name in ('SportsCenter 2017-10-17 on ESPN','Jimmy Kimmel Live! 10/16 11:35P M (62) on ABC','Jimmy Kimmel Live! 10/17 11:35P T (62) on ABC','Jimmy Kimmel Live! 10/18 11:35P W (62) on ABC','Jimmy Kimmel Live! 10/19 11:35P R (62) on ABC','Jimmy Kimmel Live! 10/20 11:35P F (62) on ABC','Jimmy Kimmel Live! 2017-10-16 on ABC','Jimmy Kimmel Live! 2017-10-17 on ABC','Jimmy Kimmel Live! 2017-10-18 on ABC','Jimmy Kimmel Live! 2017-10-19 on ABC','Jimmy Kimmel Live! 2017-10-20 on ABC','Jimmy Kimmel Live! on ABC' )
group by node_name""")
...
on a.node_name = b.node_name
""")
personified_numbers.show(20,false)
