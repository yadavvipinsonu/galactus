...
val venti_df = spark.read.parquet("s3://csxpdev-xp-east/etv/imports/venti_data_final").filter($"start_dt" >= "2016-11-21" && $"start_dt" <= "2016-11-27" && $"network_group" === "NBC (TV)").createOrReplaceTempView("venti_df")
...
spark.sql("select count(*) from venti_df").show()
val endCountTime = System.nanoTime()
val countTime = (endCountTime - startCountTime)/1000000000
println(s"time taken by s3 to read is $readTime, to count is $countTime and to execute sql query  is $runTimeQuery seconds")
spark.stop	
  }
}